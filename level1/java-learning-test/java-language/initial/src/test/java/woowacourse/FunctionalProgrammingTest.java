        ...
        void User의_나이를_기준으로_정렬한다() {
            ...
            for (int i = 0, end = users.size(); i < end; i++) {
                for (int j = i + 1; j < end; j++) {
                    ...
                    // 람다로 구현
                    /*
                    final Comparator comparator = (final Object o1, final Object o2) -> {
                        return ((User) o2).age() - ((User) o1).age();
                    };*/

                    final Comparator<User> comparator = (final User user1, final User user2) -> user1.age() - user2.age();
                    final Comparator<User> comparator1 = Comparator.comparingInt(User::age);
                    int result = comparator1.compare(users.get(i), users.get(j));
                    if (result > 0) {
    ...
    class StreamTest {
        ...
        void 명령형으로_작성된_코드를_선언형으로_변경한다() {
            ...
            final var maxAgeCrew = crews.stream()
                    ...
                    .max(Comparator.comparingInt(crew -> crew.age()))
